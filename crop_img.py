...
def cut_img(img_item):
    ...
    if( w > h ):
        ...
        if (w/h) < (c_w/c_h):
            print(1)
            im.thumbnail((c_w, c_w), Image.ANTIALIAS)
            ...
            print(2)
            #after_width = round(c_h/h*w)
            after_width = c_h/h*w
            after_width = math.ceil(c_h/h*w)
            im.thumbnail((after_width, after_width), Image.ANTIALIAS)
            im.save(dir_name + thumb_dir + img_item, 'JPEG', quality=100)
        ...
        if (w/h) < (c_w/c_h):
            print(3)
            after_height = math.ceil(c_w/w*h)
            ...
            print(4)
            im.thumbnail((c_h, c_h), Image.ANTIALIAS)
            im.save(dir_name + thumb_dir + img_item, 'JPEG', quality=100)
